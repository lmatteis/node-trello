...
var query_sprints = function(api_key, api_token, board_id, callback) {
	...
	query_cards_and_actions(api, board_id, function(err, lists) {
		if(err) { 
			...
			var sprint = find_sprint(lists, board_id);
...
var find_sprint = function(lists, board_id) {
	var start;
	var end;
	lists.forEach(function(list) {
	...
	if(!start) throw new Error('Couldnt extract sprintrange for board id ' + board_id);
