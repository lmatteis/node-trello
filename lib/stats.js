...
var passed_date = new Date(2013, 1-1, 01);
				...
				if(actions) {
					...
					for(id in members_on_card) {
						...
						if(members_on_card[id].added) {
							...
							new_card.notes.push('Couldnt find a addMemberToCard. Using start_time from card. ');
						...
						if(members_on_card[id].removed) {
							...
							new_card.notes.push('Couldnt find a removeMemberFromCard. Using finish time from card.');
	...
	async.forEach(
		...
		function(card, callback2) {
			api.get('/1/cards/' + card.card_id, function(err, response) {
				...
				card.feature_area = '<unknown>';
				var match = card.card_name.match(/.*\[(.*)].*/);
				if(match) card.feature_area = match[1];
