			...
			api.get('/1/cards/' + card.card_id + '/actions', {filter:'addMemberToCard,removeMemberFromCard'}, function(err, actions) {
				...
				if(actions) {
					for (var j = 0; j < actions.length; j++) {
						...
						if(member_on_card == null) {
							member_on_card = {
								'id': action.member.id, 
								'name' : action.member.fullName,
								'added' : card.started, //default value
								'removed': card.finished //default value
								};
						}
						if(action['type'] == 'addMemberToCard') {
							//TODO check if we allready have an addMemberToCard for this member
							member_on_card.added = new Date(action.date);
						}
						if(action['type'] == 'removeMemberFromCard') {
							member_on_card.removed = new Date(action.date);
						}
				...
				for(id in members_on_card) {
					var newCard = cloneWithMember(card, members_on_card[id]);
					console.log(newCard);
					data2.push(newCard);
				}
				callback2(null);
